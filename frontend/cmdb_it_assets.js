/* CMDB - IT 资产列表（含 CSV 导入、批量盘点/删除） */
(function () {
    window.NC.registerPage('cmdb_it_assets', {
        mixins: [window.NC.SF_MIXIN],
        template: `
        <div class="cmdb-page">
          <CmdbPageHeader title="IT 资产" subtitle="电脑 / 服务器 / 网络设备台账">
        ...
        data() { return { search: '', allRows: [], page: 1, size: 10,
            selected: [], batchVisible: false, batchDate: '', batchSaving: false,
            exporting: false, importing: false, csvImporting: false }; },
        computed: {
            searched() {
        ...
        methods: {
            ...
            async load() {
                try {
                    ...
                    const baseParams = { size: PAGE_SIZE, category: 'IT设备' };
            ...
            async refreshAll() { await this.load(); },
