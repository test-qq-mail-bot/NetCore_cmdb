...
系统信息（config.system_info）仅存储 IP/登录方式/端口/账号/备注，不再保存密码。
"""
import json
import os
import sqlite3
...
def list_assets(page: int = 1, size: int = 20, search: str = None,
    ...
    try:
        ...
        if search:
            # 审查报告 问题8：LIKE 模式注入——转义 % 和 _ 通配符
            _esc = search.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
            wheres.append("(name LIKE ? ESCAPE '\\' OR asset_no LIKE ? ESCAPE '\\' OR user LIKE ? ESCAPE '\\' OR dept LIKE ? ESCAPE '\\' OR location LIKE ? ESCAPE '\\' OR sn LIKE ? ESCAPE '\\' OR color LIKE ? ESCAPE '\\' OR storage LIKE ? ESCAPE '\\' OR memory LIKE ? ESCAPE '\\')")
            like = "%" + _esc + "%"
...
def update_asset(asset_id: int, data: dict) -> bool:
    ...
    try:
        ...
        if "config" in data:
            ...
            params.append(json.dumps(data["config"] or {}, ensure_ascii=False))
        if fields:
